...
describe("BaseController unit", () => {
  ...
  test("getByPostId returns data", async () => {
    const model = { find: jest.fn().mockResolvedValue([{ _id: "1" }]) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" } };
    const res = makeRes();

    await controller.getByPostId(req, res);

    expect(model.find).toHaveBeenCalledWith({ postID: "p1" });
    expect(res.json).toHaveBeenCalledWith([{ _id: "1" }]);
  });

  test("getByPostId returns 500 on model error", async () => {
    const model = { find: jest.fn().mockRejectedValue(new Error("db")) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" } };
    const res = makeRes();
    ...
    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.send).toHaveBeenCalledWith("Error: Can't retrieve entities by post ID");
  });

  test("createByPostId sets postID from route and creates entity", async () => {
    const model = { create: jest.fn().mockResolvedValue({ _id: "1", postID: "p1" }) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" }, body: { any: "field" } };
    const res = makeRes();

    await controller.createByPostId(req, res);

    expect(model.create).toHaveBeenCalledWith({ any: "field", postID: "p1" });
    expect(res.status).toHaveBeenCalledWith(201);
  });

  test("createByPostId returns 500 when create fails", async () => {
  ...
  test("delByPostId returns 404 when nothing deleted", async () => {
    const model = { deleteMany: jest.fn().mockResolvedValue({ deletedCount: 0 }) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" } };
    const res = makeRes();

    await controller.delByPostId(req, res);

    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.send).toHaveBeenCalledWith("Entity not found for the post");
  });

  test("delByPostId returns 200 when entities deleted", async () => {
    const deleted = { deletedCount: 2 };
    const model = { deleteMany: jest.fn().mockResolvedValue(deleted) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" } };
    const res = makeRes();
  ...
  test("delByPostId returns 500 on delete error", async () => {
    const model = { deleteMany: jest.fn().mockRejectedValue(new Error("db")) };
    const controller = new BaseController(model);
    const req: any = { params: { postId: "p1" } };
    const res = makeRes();
    ...
    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.send).toHaveBeenCalledWith("Error: Can't delete entities for the post");
  });
